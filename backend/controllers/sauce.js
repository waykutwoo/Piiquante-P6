...
exports.createSauce = (req, res, next) => {
  ...
  // Vérifiez si le type de fichier est image/jpeg ou image/png
  if (req.file.mimetype === "image/jpeg" || req.file.mimetype === "image/png") {
    ...
    sauce
      .save()
      // Si création réussie, envoi d'un message (statut 201, indique la création d'une nouvelle ressource)
      .then(() => res.status(201).json({ message: "Objet enregistré !" }))
      // Sinon renvoi d'un message d'erreur 400 (Bad Request)
      .catch((error) => res.status(400).json({ error }));
  } else {
    // Envoi d'une réponse d'erreur avec un message approprié
    res
      .status(400)
      .json({ error: "Invalid file type. Only JPEG and PNG are allowed." });
  }
};
